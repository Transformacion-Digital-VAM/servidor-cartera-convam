...
const actualizarEstadoSolicitud = async (req, res) => {
  ...
  try {
    ...
    const estadosPermitidos = ['PENDIENTE', 'APROBADO', 'RECHAZADO', 'ENTREGADO'];
    
    if (!estadosPermitidos.includes(estado_solicitud)) {
      return res.status(400).json({
        error: 'Estado no válido',
        detalle: `Los estados permitidos son: ${estadosPermitidos.join(', ')}`
      });
    }
    ...
    res.json({
      success: true,
      message: `Estado de la solicitud actualizado a ${estado_solicitud}`,
      data: result.rows[0]
    });
